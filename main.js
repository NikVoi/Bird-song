let countScore = 0, testId = 0, araryCount = 0

const scoreBlock = document.querySelector(".score"),
levelWrapper = document.querySelector(".level__wrapper"),
levelItems = document.querySelectorAll(".level__item"),
lables = document.querySelectorAll(".radio__lable"),
answerBlock = document.querySelector(".answer"),
questionImg = document.querySelector(".question__img"),
questionName = document.querySelector(".question__name"),
...
secondmas = [ 
    ...
    },
    {
      ...
      image: 'https://faunistics.com/wp-content/uploads/2021/08/4-2.jpg',
...
function audioPlay () {
    ...
    buttonAction = document.querySelector(".audio__action");

    // play audio on click button
    document.querySelector(".audio__button").addEventListener("click", play => {
    ...
    // volum for audio
    volumInput = document.querySelector(".audio__volum-input").oninput = audioVolume;
    
    function audioVolume () {
        let volumeAudio = this.value;
        audio.volume = volumeAudio / 100;
    }
    
    // progress bar for audio
    progressInput = document.querySelector(".audio__progress-input");
    
    audio.ontimeupdate = progressRoad;
    
    function progressRoad(){
        let fulltime = audio.duration,
        nowTime = audio.currentTime;    
        progressInput.value = 100 * nowTime / fulltime;
    
        document.querySelector(".audio__time-now").innerHTML = Math.ceil(nowTime);
        document.querySelector(".audio__time-full").innerHTML = Math.ceil(fulltime);
    }
}

function audioPlayListen () {
    audio = document.querySelector("#audioSecond"),
    buttonAction = document.querySelector("#audiAction");

    document.querySelector("#audioButton").addEventListener("click", play => {
        if (!buttonAction.classList.contains('active')){
            buttonAction.classList.add('active')
            audio.play();
        } else {
...
start(firstmas)
let countQuestio = 1, countLevel = 0

function start (array) {

    let bird = birdSelection(array)
    console.log(bird);
    addBirdSound (bird)
    const inputWrapper = document.querySelector(".input__wrapper")

    createInput(array, inputWrapper)
    const items = document.querySelectorAll(".input__item")

    audioPlay()
  
    showInfoAll(inputWrapper, items, array, bird)
}
...
function addBirdSound (object) {
    const audioHTML = document.querySelector(".audio__HTML")

    const blockAudio = `<audio src="${object.audio}" id="audio"></audio>`

    audioHTML.innerHTML = blockAudio
}

function birdSelection (array) {
    min = Math.ceil(1);
    max = Math.floor(6);
...
function searchTrueAswer (inputWrapper, inputItem, object, array) {
    let mainId = 0;
    ...
    inputItem.forEach(item => {
        item.addEventListener("click", search => {
            if (mainId == object.id){
                ...
                item.childNodes[2].classList.add('true')
                score()
                playTrue()
                showHead(object)
                showInfo(array, item.id)
                audioPlayListen()

                buttonClick(secondmas)
            } else {
                item.childNodes[2].classList.add('false')
                showInfo(array, item.id)
                audioPlayListen()
                playFalse()
            }
...
function createInput (array, inputWrapper) {
    let test = ''

    array.forEach(item => {
        const anwerInput = `
            <div class="input__item" id="${item.id}">
                <input class="input__radio" type="radio"><label class="radio__lable" for="input">${item.name}</label>
            </div>
        `
        test += anwerInput
    })

    inputWrapper.innerHTML = test
}

function showInfo (array, id) {
    const answer = document.querySelector(".answer__info")

    array[--id]
                    
    const showBlock = `<div class="answer__wrapper active">
                        <div class="question__img">
                            <img src="${array[id].image}" alt="bird">
                        </div>
                        <div class="question__wrapper">
                            ...
                            <hr>
                            <div class="audio">
                            <div class="audio__HTML">

                            </div>
                            <div class="audio__button" id="audioButton">
                                <div class="audio__action" id="audiAction"></div>
                            </div>
                            <div class="audio__progress">
                                <audio src="${array[id].audio}" id="audioSecond"></audio>

                                <input type="range" min="0" max="100" value="1" class="audio__progress-input" id = "audio__progress-input">

                                <div class="audio__time">
                                    <div>00:<span class="audio__time-now" id="audio__time-now">00</span></div>
                                    <div>00:<span class="audio__time-full" id="audio__time-full">00</span></div>
                                </div>
                            </div>
                            <div class="audio__volum"><input type="range" min="0" max="100" class="audio__volum-input" id = "audio__volum-input"></div>
                            </div>
                        </div>
                    </div>

                    <div class="answer__description active">
                        ${array[id].description}
                    </div>
    `
   answer.innerHTML = showBlock;
}

function showInfoAll (inputWrapper, inputItem, array, object) {
        searchTrueAswer (inputWrapper, inputItem, object, array)
}

function showTrueAnswer (array, id) {
    const imgBlock = document.querySelector(".question__img")
    const questionBlock = document.querySelector(".question__name")
    array[--id]

    const imgShow = `<img src="${array[id].image}" alt="bird">`

    imgBlock.innerHTML = imgShow
    questionBlock.innerHTML = array[id].name
}

function showHead (object) {
    const block =`<img src="${object.image}" alt="bird"></img>` 
    document.querySelector(".question__name").innerHTML = object.name
    document.querySelector(".question__img").innerHTML = block
}

function buttonClick (array) {
    const resetQuestion = `<img src="./img/logo.webp" alt="bird">`
    const resetAnswer = `
    <div class="input__wrapper">
    </div>

    <div class="answer__info">
        <p class="active">Послушайте плеер. <br> Выберите птицу из списка</p>
    </div>
    `
    const resetAudio = `
    <div class="audio__button">
    <div class="audio__action"></div>
</div>
<div class="audio__progress">
    <input type="range" min="0" max = "100" value="1" class="audio__progress-input">

    <div class="audio__time">
        <div>00:<span class="audio__time-now">00</span></div>
        <div>00:<span class="audio__time-full">00</span></div>
    </div>
</div>
<div class="audio__volum"><input type="range" min = 0 max = 100 class="audio__volum-input"></div>
    `
    ...
    buttonNext.classList.remove("noActive")
    
    if (countQuestio < 7){
        buttonNext.addEventListener("click", foo => {
            ...
            levelItems[countLevel].classList.add("active")
            buttonNext.classList.add("noActive")

            questionImg.innerHTML = resetQuestion
            questionName.innerHTML = "****"
            answerBlock.innerHTML = resetAnswer
            audioBlock.innerHTML = resetAudio

            if (araryCount === 0){
                console.clear()
                second(array[0])
                
            } else if (araryCount === 1) {
                console.clear()
                second(array[0])
            } else if (araryCount === 2) {
                console.clear()
                second(array[1])
            } else if (araryCount === 3) {
                console.clear()
                second(array[2])
            } else if (araryCount === 4) {
                console.clear()
                second(array[3])
            } else if (araryCount === 5) {
                console.clear()
                second(array[4])
            }
        })
        ...
        scoreModal = document.querySelector(".modal__score")

        modal.classList.add("active")

        scoreModal.innerHTML = countScore
    }
}

function score () {
    const error = document.querySelectorAll(".input__item")

    let number = 5

    error.forEach(item => {
        if (item.childNodes[2].classList.contains("false"))
            number--
    })

    countScore += number
    
    scoreBlock.innerHTML = countScore
}

function playTrue() {
    const path = "./audio/zvuk-pravilnogo-otveta-iz-peredachi-100-k-1.mp3"
    let audio = new Audio(path);
    audio.play();
}

function playFalse() {
    const path = "./audio/golosom-ee-ne-to.mp3"
    let audio = new Audio(path);
    audio.play();
}
